...
def main():
    ...
    st.warning(
        "Enter a SMILES string of a molecule in the sidebar and choose a regressor model(s) to predict its column density. Some examples are provided below."
    )
    ...
    with st.sidebar:
        ...
        molecules_name = st.text_input(
            f'Enter molecule in "{mol_representation}" format', "CCC#N, C#CC#[O+], CC1CCCCC1, Cc1ccccc1"
        )
        if molecules_name == "":
    ...
    with st.sidebar:
        regressor_model_lists = st.multiselect(
            "Choose a regressor model", regressor_list, default=["gbr", "svr", "rfr"]
        )
